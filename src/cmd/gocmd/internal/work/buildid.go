...
func (b *Builder) toolID(name string) string {
	...
	cmd := base.ToolCommand(cmdline[0], cmdline[1:]...)
...
func assemblerIsGas() bool {
	cmd := base.ToolCommand(BuildToolchain.compiler(), "-print-prog-name=as")
	assembler, err := cmd.Output()
	if err == nil {
		cmd := base.ToolCommand(strings.TrimSpace(string(assembler)), "--version")
		out, err := cmd.Output()
